...
export default function Tasks(props : tasksType) {
    ...
    return (
        <div className='mt-[2rem] h-full flex flex-col '>
            {tasks?.map((tasks : taskType, index : number) => (
                <div  className='mb-10 cursor-pointer flex 
                shadow-md  justify-start  text-[rgb(0,0,0)] 
                dark:text-[rgba(255,255,255,255)] pr-1 pt-2 rounded-md 
                pl-2 min-h-[5rem] w-[17rem] dark:bg-[rgba(43,44,55,255)] 
                bg-[rgba(255,255,255,255)] ' key={`${tasks.title}_fdev_${index}`}>
                    <div className='pb-5' key={`${index}__${tasks.title}`}>
